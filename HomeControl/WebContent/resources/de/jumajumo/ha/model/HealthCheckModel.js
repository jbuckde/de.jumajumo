...
sap.ui.model.json.JSONModel.extend("jumajumo.ha.model.HealthCheckModel",
{
	loadData : function()
	{
		var that = this;
	...
	_isGarageDoorSwitchActive : function()
	{
		...
		jQuery.ajax(
		{
			url : "dispatcher/trigger/garagedevicehealthcheck/activate",
			async : true,
			dataType : "json",
			type : "GET",
			error : function(data, sStatus, sError)
			{
				that.setProperty("/garageDoorSwitchActive", sStatus);
			},
			success : function(data, sStatus)
			{
				that.setProperty("/garageDoorSwitchActive", data[0].success);
			},
